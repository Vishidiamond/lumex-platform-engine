...
/** After children mount and bounds become non-zero, position the camera so the
 *  full bounding box fits the viewport with a small margin. */
function AutoFit({ children, margin = 1.3 }: { children: React.ReactNode; margin?: number }) {
  ...
  useEffect(() => {
    ...
    const attempt = () => {
      ...
      if (bsize.length() > 1) {
        ...
        const dist = Math.max(fitHeightDist, fitWidthDist, bsize.z / 2) * margin;

        persp.position.set(center.x, center.y, center.z + dist);
        persp.near = Math.max(0.01, dist / 1000);
        persp.far = dist * 20;
        persp.lookAt(center);
      ...
      if (tries < 240) raf = requestAnimationFrame(attempt);
    };
